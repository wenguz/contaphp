...
function ChapterTitle($label,$fecha)
{
    ...
    $this->SetX((210-$w)/2);
    // Título
    $this->Cell($w,6,"Estado de resultados",0,1,'C');
...
function FancyTable($header, $data)
{
    ...
    $this->SetFillColor(227,231,100);//color de la celda
    ...
    $fill = !$fill;
     ...
     $this->SetY(-29);
     $this->Cell(0,3,'TOTAL '.number_format(array_sum($row)),'',0,'',$fill);
...
$header = array('INGRESO', 'DEBE', 'HABER');
// Carga de datos
$data = $pdf->LoadData('reportes_txt/est_resultado.txt');
